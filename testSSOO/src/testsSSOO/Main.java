...
public class Main {
    ...
    public static void iniciarTest() {
        ...
        JFrame frame = new JFrame("Simulador de Examen de Sistemas Operativos");
        ...
        for (int i = 0; i < 5; i++) {
            ...
            botonesOpciones[i].addActionListener(e -> {
                if (!respondidas[preguntaActual]) {
                    String respuestaSeleccionada = botonesOpciones[indice].getText();
                    preguntas[preguntaActual].setIndiceSeleccionado(indice);
                    if (respuestaSeleccionada.equals(preguntas[preguntaActual].getSolucion())) {
                        ...
                        for (JButton boton : botonesOpciones) {
                            if (boton.getText().equals(preguntas[preguntaActual].getSolucion())) {
    ...
    public static void mostrarPantallaFinal() {
        ...
        double notaFinal = (10.0 / (totalPreguntas - 8)) * (aciertos - 8);
    ...
    public static void mostrarPregunta(JLabel etiquetaPregunta, JButton[] botonesOpciones, JLabel etiquetaNum) {
		...
		botonesOpciones[4].setText(pregunta.getCuestion5());
		botonesOpciones[4].setFont(new Font("Arial", Font.PLAIN, 16));
        ...
        if (respondidas[preguntaActual]) {
            ...
            if (seleccionado != -1) {
                if (pregunta.isRespondidaCorrectamente()) {
                    ...
                    for (JButton boton : botonesOpciones) {
                        if (boton.getText().equals(pregunta.getSolucion())) {
                            boton.setBackground(verdeSuave);
                            break;
                        }
                    }
    ...
    public static Pregunta[] cuestionario() {
        ...
        while (cuenta < preguntas.length) {
            int escogida = 0;
            try {
                File fichero = new File("preguntasSSOO.txt");
                ...
                while (lectura.hasNext()) {
                    ...
                    String pregunta5 = lectura.nextLine();
                    int add = 1;
                    ...
                    if (add == 1 && rng == 0 && cuenta < preguntas.length) {
                    	...
                    	cuestion = cuestion.replace("<", "&lt;");
                    	cuestion = cuestion.replace(">", "&gt;");
                        Pregunta test = new Pregunta(cuestion, solucion, solucion, pregunta2, pregunta3, pregunta4, pregunta5);
                        test.reorganizar();
